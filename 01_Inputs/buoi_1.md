...
Đề: Cho hàm số $y=f(x)$ xác định trên $\mathbb{R}$ thỏa mãn $\lim _{x \rightarrow 3} \frac{f(x)-f(3)}{x-3}=2$. Kết quả đúng là:
A. $f^{\prime}(2)=3$.
B. $f^{\prime}(x)=2$.
C. $f^{\prime}(x)=3$.
D. $f^{\prime}(3)=2$.
Lời giải: Theo định nghĩa đạo hàm của hàm số tại một điểm ta có: $\lim _{x \rightarrow 3} \frac{f(x)-f(3)}{x-3}=f^{\prime}(3) = 2$.

## CÂU 2  | level: tb | đáp án: B
